...
function getWatchUrl(url) {
  // Para Google Drive, convertir a link de vista previa
  const driveMatch = url?.match(/\/d\/([^/]+)/)
...
function TutorialCard({ tutorial }) {
  ...
  function handleClick() {
    if (ytId || isDrive) {
      ...
      window.open(tutorial.drive_url, '_blank')
    }
  }
    ...
    >
        ...
        {showPlayer ? (
          ...
          <>
            {thumb && !thumbErr
              ...
              : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `linear-gradient(135deg, #5822b415, #5822b408)` }}>
                  <svg width={48} height={48} viewBox="0 0 24 24" fill="none" stroke="#5822b4" strokeWidth="1.5" opacity="0.5"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                </div>
            }
            {/* Overlay play */}
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        ...
        {tutorial.description && (
          <p style={{ fontSize: 12, color: 'var(--ink-muted)', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
            {tutorial.description}
          </p>
        )}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#5822b4' }} />
          <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#5822b4' }}>Tutorial</span>
        </div>
      </div>
    </div>
  )
...
export default function ProveedorTutoriales() {
  const [tutorials, setTutorials] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    supabase.from('tutorials').select('*')
      .eq('target_role', 'proveedor').eq('is_active', true)
    ...
    <div>
      ...
      {tutorials.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px 24px', background: 'var(--surface-raised)', borderRadius: 16, border: '1px solid var(--surface-border)' }}>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: '#5822b415', border: '1.5px solid #5822b430', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <svg width={24} height={24} viewBox="0 0 24 24" fill="none" stroke="#5822b4" strokeWidth="2"><polygon points="5 3 19 12 5 21 5 3"/></svg>
          </div>
